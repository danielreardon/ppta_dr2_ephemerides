
        \begin{table}
        \footnotesize
        \begin{tabular}{llllllll}
        \hline\hline \noalign{\vskip 1.5mm}
        Pulsar Name 	 & 	 J0613$-$0200	 & 	 J1017$-$7156	 & 	 J1022+1001	 & 	 J1045$-$4509 
 \\ \hline \noalign{\vskip 1.5mm} 
Number of TOAs\dotfill	 & 	 $6044$	 & 	 $4053$	 & 	 $7612$	 & 	 $5807$\\ 
Number of observations\dotfill	 & 	 $1154$	 & 	 $736$	 & 	 $1166$	 & 	 $1100$\\ 
MJD range\dotfill	 & 	 $51526$ -- $58230$	 & 	 $55393$ -- $58232$	 & 	 $52649$ -- $58230$	 & 	 $49405$ -- $58212$\\ 
Right ascension (RA), $\alpha$ (hh:mm:ss)\dotfill	 & 	 $6$:$13$:$43.975902(2)$	 & 	 $10$:$17$:$51.321896(6)$	 & 	 $10$:$22$:$57.995(1)$	 & 	 $10$:$45$:$50.18519(1)$\\ 
Declination (DEC), $\delta$ (dd:mm:ss)\dotfill	 & 	 $-2$:$0$:$47.24352(8)$	 & 	 $-71$:$56$:$41.61771(3)$	 & 	 $10$:$1$:$52.69(4)$	 & 	 $-45$:$9$:$54.1062(1)$\\ 

 \noalign{\vskip 1.5mm} 
Spin frequency, $f$ (${\rm s}^{-1}$)\dotfill	 & 	 $\mathbf{ 326.600561967271(1) }$	 & 	 $\mathbf{ 427.6219050534548(8) }$	 & 	 $\mathbf{ 60.7794479478993(2) }$	 & 	 $\mathbf{ 133.793149524054(1) }$\\ 
First spin frequency derivative, ${\dot{f}}$ (${\rm s}^{-2}$)\dotfill	 & 	 $\mathbf{ -1.02299(2)\times 10^{-15} }$	 & 	 $\mathbf{ -4.0493(2)\times 10^{-16} }$	 & 	 $\mathbf{ -1.60100(2)\times 10^{-16} }$	 & 	 $\mathbf{ -3.16195(9)\times 10^{-16} }$\\ 
Dispersion measure, DM (${\rm cm}^{-3}\,{\rm pc}$)\dotfill	 & 	 $\mathbf{ 38.762(5) }$	 & 	 $\mathbf{ 94.2233(4) }$	 & 	 $\mathbf{ 10.2444(3) }$	 & 	 $\mathbf{ 58.149(2) }$\\ 
Ecliptic latitude $\beta$ (deg.)\dotfill	 & 	 $-$	 & 	 $-$	 & 	 $-$	 & 	 $-$\\ 
Ecliptic longitude $\lambda$ (deg.)\dotfill	 & 	 $-$	 & 	 $-$	 & 	 $-$	 & 	 $-$\\ 

 \noalign{\vskip 1.5mm} 
Proper motion in RA, $\mu_\alpha \cos\delta$ (${\rm mas}\,{\rm yr}^{-1}$)\dotfill	 & 	 $-$	 & 	 $-$	 & 	 $-$	 & 	 $-$\\ 
Proper motion in DEC, $\mu_\delta$ (${\rm mas}\,{\rm yr}^{-1}$)\dotfill	 & 	 $-$	 & 	 $-$	 & 	 $-$	 & 	 $-$\\ 
Proper motion in ecliptic latitude, $\mu_\beta$ (${\rm mas}\,{\rm yr}^{-1}$)\dotfill	 & 	 $-$	 & 	 $-$	 & 	 $-$	 & 	 $-$\\ 
Proper motion in ecliptic longitude, $\mu_\lambda \cos\beta$ (${\rm mas}\,{\rm yr}^{-1}$)\dotfill	 & 	 $-$	 & 	 $-$	 & 	 $-$	 & 	 $-$\\ 
Parallax, $\pi$ (${\rm mas}$)\dotfill	 & 	 $\mathbf{ 1.01(9) }$	 & 	 $\mathbf{ 0.7(6) }$	 & 	 $\mathbf{ 1.6(2) }$	 & 	 $\mathbf{ 1.4(7) }$\\ 

 \noalign{\vskip 1.5mm} 
Obital period, $P_{\mathrm{b}}$ ($\mathrm{d}$)\dotfill	 & 	 $\mathbf{ 1.19851257517(4) }$	 & 	 $\mathbf{ 6.5119040(9) }$	 & 	 $\mathbf{ 7.8051353(6) }$	 & 	 $\mathbf{ 4.08352925453(8) }$\\ 
Projected semimajor axis, $x$ (lt-s)\dotfill	 & 	 $\mathbf{ 1.09144422(4) }$	 & 	 $\mathbf{ 4.83003(8) }$	 & 	 $\mathbf{ 16.765405(2) }$	 & 	 $\mathbf{ 3.0151325(5) }$\\ 
Epoch of periastron, $T_0$ (MJD)\dotfill	 & 	 ${ 50324.51 } ^{ +0.15 }_{ -0.14 }$	 & 	 $\mathbf{ 55335.061(8) }$	 & 	 $\mathbf{ 49778.4079(5) }$	 & 	 $50196.8(1)$\\ 
Longitude of periastron, $\omega$ (deg)\dotfill	 & 	 $48.4(8)$	 & 	 $\mathbf{ 329.5(4) }$	 & 	 $\mathbf{ 97.64(2) }$	 & 	 $-118.0(2)$\\ 
Eccentricity of orbit, $e$\dotfill	 & 	 $5.24(7)\times 10^{-6}$	 & 	 $\mathbf{ 0.0001424(6) }$	 & 	 $\mathbf{ 9.703(4)\times 10^{-5} }$	 & 	 ${ 2\times 10^{-5} } ^{ +8\times 10^{-8} }_{ -9\times 10^{-8} }$\\ 

 \noalign{\vskip 1.5mm} 
Epoch of ascending node, $T_{\mathrm{ASC}}$ (MJD)\dotfill	 & 	 $\mathbf{ 50315.26949111(8) }$	 & 	 $-$	 & 	 $-$	 & 	 $\mathbf{ 50273.5070048(1) }$\\ 
First Laplace parameter, $\epsilon_1 = e \sin \omega$\dotfill	 & 	 $\mathbf{ 3.92(7)\times 10^{-6} }$	 & 	 $-$	 & 	 $-$	 & 	 $\mathbf{ -2.073(9)\times 10^{-5} }$\\ 
Second Laplace parameter, $\epsilon_2 = e \cos \omega$\dotfill	 & 	 $\mathbf{ 3.48(7)\times 10^{-6} }$	 & 	 $-$	 & 	 $-$	 & 	 $\mathbf{ -1.102(9)\times 10^{-5} }$\\ 
First time derivative of orbital period, ${\dot P}_{\mathrm{b}}$ \dotfill	 & 	 $\mathbf{ 2.1(7)\times 10^{-14} }$	 & 	 $\mathbf{ 4(2)\times 10^{-13} }$	 & 	 $-$	 & 	 $-$\\ 
Rate of advance of periastron, ${\dot \omega}$ (deg\,yr$^{-1}$)\dotfill	 & 	 $-$	 & 	 $\mathbf{ 0.016(3) }$	 & 	 $\mathbf{ 0.011(1) }$	 & 	 $-$\\ 

 \noalign{\vskip 1.5mm} 
Rate of change of projected semi-major axis, ${\dot x}$ \dotfill	 & 	 $-$	 & 	 $\mathbf{ 7.6(6)\times 10^{-15} }$	 & 	 $\mathbf{ 1.33(6)\times 10^{-14} }$	 & 	 $\mathbf{ -3(1)\times 10^{-15} }$\\ 
Sine of inclination angle, $\sin i$\dotfill	 & 	 $-$	 & 	 $-$	 & 	 $-$	 & 	 $-$\\ 
Companion mass, $M_{\mathrm{c}}$ ($M_{\odot}$)\dotfill	 & 	 ${ 0.1 } ^{ +0.12 }_{ -0.05 }$	 & 	 ${ 0.5 } ^{ +2 }_{ -0.4 }$	 & 	 ${ 0.52 } ^{ +0.3 }_{ -0.16 }$	 & 	 $-$\\ 
Orthometric Shapiro delay parameter, $h_3$ ($\mu\,$s)\dotfill	 & 	 $\mathbf{ 2.4(3)\times 10^{-7} }$	 & 	 $\mathbf{ 1.1(3)\times 10^{-7} }$	 & 	 $\mathbf{ 6.3(6)\times 10^{-7} }$	 & 	 $-$\\ 
Orthometric Shapiro delay parameter, $h_4$ ($\mu\,$s)\dotfill	 & 	 $\mathbf{ 1.9(4)\times 10^{-7} }$	 & 	 $-$	 & 	 $-$	 & 	 $-$\\ 

 \noalign{\vskip 1.5mm} 
Orthometric Shapiro delay parameter, $\zeta = h_4 / h_3$\dotfill	 & 	 $-$	 & 	 $\mathbf{ 0.2(3) }$	 & 	 $\mathbf{ 0.63(8) }$	 & 	 $-$\\ 
Longitude of ascending node, $\Omega$ (deg.)\dotfill	 & 	 $-$	 & 	 $-$	 & 	 $-$	 & 	 $-$\\ 
Inclination angle, $i$ (deg.)\dotfill	 & 	 $76^{ +10 }_{ -13 }$	 & 	 $39^{ +20 }_{ -17 }$	 & 	 $64^{ +6 }_{ -7 }$	 & 	 $-$\\ 
Upper limit on $i$ (deg.)\dotfill	 & 	 $-$	 & 	 $<44$	 & 	 $<73$	 & 	 $<50$\\ 
Parallax distance, $D_\pi$ (kpc)\dotfill	 & 	 ${ 0.99 } ^{ +0.1 }_{ -0.08 }$	 & 	 ${ 1.2 } ^{ +1.7 }_{ -0.5 }$	 & 	 ${ 0.64 } ^{ +0.07 }_{ -0.06 }$	 & 	 ${ 0.7 } ^{ +0.6 }_{ -0.2 }$\\ 

 \noalign{\vskip 1.5mm} 
Shklovskii distance $D_{\rm shk}$ (kpc)\dotfill	 & 	 $0.6(3)$	 & 	 $3(1)$	 & 	 $-$	 & 	 $-$\\ 
Pulsar mass, $M_{\mathrm{P}}$ ($M_{\odot}$) \dotfill	 & 	 ${ 0.9 } ^{ +1.7 }_{ -0.6 }$	 & 	 ${ 2.64 } ^{ +16 }_{ -2 }$	 & 	 ${ 0.6 } ^{ +0.5 }_{ -0.3 }$	 & 	 $-$\\ 
Total system mass $M_{\rm tot}$ ($M_{\odot}$)\dotfill	 & 	 ${ 1.0 } ^{ +1.8 }_{ -0.7 }$	 & 	 $3^{ +18 }_{ -3 }$	 & 	 ${ 1.1 } ^{ +0.7 }_{ -0.4 }$	 & 	 $-$\\ 
Transverse velocity, $v_t$ (km\,s$^{-1}$)\dotfill	 & 	 $49^{ +5 }_{ -4 }$	 & 	 $59^{ +80 }_{ -20 }$	 & 	 $54^{ +12 }_{ -7 }$	 & 	 $26^{ +20 }_{ -9 }$\\ 

        \noalign{\vskip 1.5mm}
        \hline\hline
        \end{tabular}\hfill\
        \caption{\label{tab:XXXXX}
        Placeholder caption.....
        }
        \end{table}
        