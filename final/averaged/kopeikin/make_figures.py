...
data_shortname = 'results/best4-J1909-3744-'
plot_params = ["M2", "KOM", "KIN"]  # plot_params = None  (If plotting everything)
...
for name in cnames:  # rename parameters for plotting
    if name in plot_params:
        if name == "PBDOT":
            new_plot_params.append("$\dot{P}_b$")
            new_names.append("$\dot{P}_b$")
        elif name == "M2":
            ...
            new_plot_params.append("$\dot{F}$")
            new_names.append("$\dot{F}$")
        elif name == "OMDOT":
            new_plot_params.append("$\dot{\omega}$")
            new_names.append("$\dot{\omega}$")
        elif name == "PX":
            new_plot_params.append("$\pi$")
            new_names.append("$\pi$")
        else:
...
plt.figure(figsize=(9, 9))
plt.hist(kom, bins=100)
plt.xlabel('kom')
plt.savefig("kom.pdf")
plt.close()

plt.figure(figsize=(9, 9))
plt.hist(kin, bins=100)
plt.xlabel('kin')
plt.savefig("kin.pdf")
plt.close()
exit()
